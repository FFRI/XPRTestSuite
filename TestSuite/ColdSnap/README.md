# ColdSnap (a.k.a, POOLRAT)

## About this scanning module

XProtectRemediatorColdSnap is a scanning module that detects POOLRAT malware.
...
let cdHashes = ["99c31f166d1f1654a1b7dd1a6bec3b935022a020", "852ba9494c42c084a131e74fc347fa0361d7c3aa"]
let processNames = ["Quartz", "UpdateAgent", "systemui", "com.docker.vmnetd.helper", "pdpaste", "com.microsoft.office.licensingV3.helper", "corplink-helper", "gklp", "sambad", "zgkp", "prtspool", "dcups", "check_update", "translate", "update", "SafariChecker", "SafariUpdateAgent", "TranslationHelper"]
let fileNames = ["/private/etc/krb5d.conf", "~/Library/WebKit/xpdf.conf"]
let dirNames = ["~/Library/Trial/", "~/Library/Translation/", "/Users/Shared/", NSTemporaryDirectory()]
...
ColdSnapRemediator {
    ...
    for dirName in dirNames {
        // enumerate is a function that enumerate files in the directory (this function name is inferred from RE. It does not reflect the actual name)
        for fileName in XPPluginDirectory(dirName).enumerate() {
            File(path: fileName) {
                MaxFileSize(204800)
                FileMacho(true)
                FileYara(YaraMatcher(yaraColdSnap))
            }
        }
    }
}
