...
XProtectRemediatorPirrit is a scanning module that detects and remediates notorious Pirrit adware samples.

<details>
<summary>Remediation logic described with RemediationBuilder DSL (click to expand)</summary>

```swift
// macos_pirrit_common_assembly detects the common obfuscated Pirrit code
// See https://speakerdeck.com/patrickwardle/armd-and-dangerous-analyzing-arm64-malware-targeting-macos?slide=37
let yaraPirrit = """
import "macho"
private rule Macho
{
...
// This YARA rule seems to detect self-extracting shell scripts.
// $magic1 is the magic number of the ZIP file.
let yaraPirritShellScript = """
...
PirritRemediator {
    Service(tag: nil) {
        // Describe conditions for keys and values in .plist files registered in LaunchAgents/LaunchDaemons
        KeyValue(["WatchPaths":.Wildcard, "StartInterval":.Wildcard, "Label":.Pattern(".*\\.app")])
        ExecutableYara(yaraPirrit)
    }
    Service(tag: nil) {
        // See SentinelLabs blog post (https://www.sentinelone.com/labs/a-threat-hunters-guide-to-the-macs-most-prevalent-adware-infections-2022/)
        // For example, ~/Library/Application Support/com.hoge/hoge.app/Contents/MacOS/hoge
        ExecutablePath(.Pattern("^.+/Library/(Application Support/com\\.)?([a-zA-Z]+)/\\2(\\.app/Contents/MacOS/\\2)?$"))
        ExecutableYara(yaraPirrit)
    }
    Service(tag: nil) {
        ExecutableYara(yaraPirritHunt)
        ExecutableYara(yaraPtraceDeny)
    }  
    Service(tag: nil) {
        ExecutableYara(yaraPirritHunt)
    }.reportOnly()
    // enumerateShFiles() is a function that enumerate files in /private/var/tmp/*.sh (this function name is inferred from RE. It does not reflect the actual name)
    for shFile in enumerateShFiles() {
        if shFile.fileSize > 0x1869f { // 100000 bytes
            File(path: shFile.path) {
                FileYara(yaraPirritShellScript)
            }.reportOnly()
        }
    }
}
...
- [A Threat Hunter’s Guide to the Mac’s Most Prevalent Adware Infections 2022](https://www.sentinelone.com/labs/a-threat-hunters-guide-to-the-macs-most-prevalent-adware-infections-2022/)
- [Arm’d & Dangerous: Analyzing arm64 Malware Targeting macOS](https://speakerdeck.com/patrickwardle/armd-and-dangerous-analyzing-arm64-malware-targeting-macos)
